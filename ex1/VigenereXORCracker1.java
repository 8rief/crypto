...
public class VigenereXORCracker1 {
    ...
    public static void getLength() {
        ...
        for (int keylen = 1; keylen < 14; keylen++) {
            for (int index = 0; index < keylen; index++) {
                List<Integer> subarr = new ArrayList<>();
                for (int i = index; i < ciArray.size(); i += keylen) {
                    subarr.add(ciArray.get(i));
                }
                List<Integer> ansKeys = findIndexKey(subarr, testChars);
                System.out.println("keylen=" + keylen + " index=" + index + " keys=" + ansKeys);
                if (!ansKeys.isEmpty()) {
                    List<Character> ch = new ArrayList<>();
                    for (int x : ansKeys) {
                        ch.add((char) (x ^ subarr.get(0)));
                    }
